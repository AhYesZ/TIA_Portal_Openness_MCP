...
# Import .scl via ImportPlcExternalSource + GenerateBlocksFromExternalSource on 安全PLC only.
# Does not touch 5T车 / 10T车.
...
$srcScl = Join-Path $tiaportalRoot 'skill\scl-cookbook\MCPVerify_FC_SCL_v2.scl'
if (-not (Test-Path -LiteralPath $srcScl)) { throw "Missing SCL file: $srcScl" }
$staging = Join-Path $env:TEMP 'tiaportal-mcp-verify'
New-Item -ItemType Directory -Force -Path $staging | Out-Null
$scl = Join-Path $staging 'MCPVerify_FC_SCL_v2.scl'
Copy-Item -LiteralPath $srcScl -Destination $scl -Force
$reportMd   = "$PSScriptRoot\verify_scl_v2.md"
$reportJson = "$PSScriptRoot\verify_scl_v2.json"
...
function Verify {
    ...
    try {
        ...
        elseif ($null -eq $resp.result) { $entry.detail = "empty result" }
            ...
            elseif ($hasOkFalse -or $hasSuccessFalse -or $hasErrorField) { $entry.detail = "logical-fail: " + ($text.Substring(0,[Math]::Min(2000,$text.Length))) }
                ...
                $entry.status = 'pass'
                $entry.detail = if ($text) { ($text.Substring(0,[Math]::Min(500,$text.Length)) -replace '\s+', ' ') } else { '' }
            }
        }
    ...
    Write-Host ("[{0,-4}] {1,-58} {2,6}ms  {3}" -f $entry.status.ToUpper(), $label, $entry.elapsedMs, ($entry.detail.Substring(0,[Math]::Min(140,$entry.detail.Length)))) -ForegroundColor $color
...
try {
    ...
    Send-Request 'initialize' @{ protocolVersion='2024-11-05'; capabilities=@{}; clientInfo=@{ name='verify-scl-v2'; version='1.0' } } 30000 | Out-Null
    ...
    Verify -category 'L2-PLC' -label "DeletePlcExternalSource(MCPVerify_FC_SCL_v2.scl)" -toolArgs @{
        softwarePath=$plc; externalSourceName='MCPVerify_FC_SCL_v2.scl'
    } -timeoutMs 60000 -toolName 'DeletePlcExternalSource' | Out-Null

    Verify -category 'L2-PLC' -label "ImportPlcExternalSource(MCPVerify_FC_SCL_v2.scl)" -toolArgs @{
        softwarePath=$plc; groupPath=''; filePath=$scl
    } -timeoutMs 60000 -toolName 'ImportPlcExternalSource' | Out-Null

    Verify -category 'L2-PLC' -label "GetPlcExternalSources($plc)" -toolArgs @{ softwarePath=$plc } -timeoutMs 30000 -toolName 'GetPlcExternalSources' | Out-Null

    Verify -category 'L2-PLC' -label "GenerateBlocksFromExternalSource(MCPVerify_FC_SCL_v2.scl)" -toolArgs @{
        softwarePath=$plc; externalSourceName='MCPVerify_FC_SCL_v2.scl'
    } -timeoutMs 180000 -toolName 'GenerateBlocksFromExternalSource' | Out-Null

    $cmp = Verify -category 'L2-PLC' -label "CompileSoftware($plc)" -toolArgs @{ softwarePath=$plc } -timeoutMs 360000 -toolName 'CompileSoftware'
    if ($cmp.pass) {
        try {
            ...
            if ($ec -gt 0) {
                [void]$script:results.Add([pscustomobject]@{
                    ...
                    detail = "errorCount=$ec (SCL import must compile clean)"
                })
            }
        } catch { }
    }
    ...
    $md = @()
    $md += "# SCL external source v2 (CASE FOR WHILE)"
    $md += ""
