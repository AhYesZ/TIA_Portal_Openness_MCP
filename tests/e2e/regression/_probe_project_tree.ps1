...
function Call($method, $params, [int]$timeoutMs=60000) {
    ...
    while ([DateTime]::UtcNow -lt $deadline) {
        ...
        try { $j = $line | ConvertFrom-Json; if ($j.id -eq $script:id) { $script:id++; return $j } } catch {}
    }
    throw "timeout"
}
...
try {
    ...
    Call 'tools/call' @{ name='Connect'; arguments=@{} } 180000 | Out-Null
    $gp = Call 'tools/call' @{ name='GetProject'; arguments=@{} } 60000
    $name = ($gp.result.content | ? { $_.type -eq 'text' } | Select -First 1).text | ConvertFrom-Json | % { $_.items[0].name }
    Write-Host "Project: $name" -ForegroundColor Cyan
    Call 'tools/call' @{ name='AttachToOpenProject'; arguments=@{ projectName=$name } } 60000 | Out-Null
    $tr = Call 'tools/call' @{ name='GetProjectTree'; arguments=@{} } 60000
    $tree = ($tr.result.content | ? { $_.type -eq 'text' } | Select -First 1).text | ConvertFrom-Json | % { $_.tree }
    Write-Host ""; Write-Host "===== PROJECT TREE =====" -ForegroundColor Cyan
    Write-Host $tree
} finally {
