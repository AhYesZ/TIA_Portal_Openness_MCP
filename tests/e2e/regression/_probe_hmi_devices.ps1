...
try {
  ...
  $tree = Call 'tools/call' @{ name='GetProjectTree'; arguments=@{} }
  Write-Host "=== ProjectTree ===" -ForegroundColor Cyan
  Write-Host (($tree.result.content[0].text | ConvertFrom-Json).tree)
  $devs = Call 'tools/call' @{ name='GetDevices'; arguments=@{} }
  Write-Host "=== Devices ===" -ForegroundColor Cyan
  Write-Host ($devs.result.content[0].text)
