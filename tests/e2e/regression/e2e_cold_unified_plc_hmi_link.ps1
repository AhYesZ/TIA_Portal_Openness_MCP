...
# Cold-start end-to-end: CreateProject -> add PLC + Unified HMI -> create PLC DB tags
# -> create Unified HMI connection + HMI tags bound to PLC symbols -> add screen + button actions
# -> compile PLC (errorCount=0) -> save.
...
$projDir = "C:\Users\XL626\Desktop\testtia\mcp-cold-unified-link_$ts"
$projName = "MCP_Cold_Unified_Link_$ts"

$reportMd   = "$PSScriptRoot\e2e_cold_unified_plc_hmi_link.md"
$reportJson = "$PSScriptRoot\e2e_cold_unified_plc_hmi_link.json"
...
try {
    ...
    Send-Request 'initialize' @{ protocolVersion='2024-11-05'; capabilities=@{}; clientInfo=@{ name='e2e-cold-unified-link'; version='1.0' } } 30000 | Out-Null
    ...
    # Hardware: PLC (S7-1200) + Unified HMI (requires WinCC Unified installed)
    ...
    if (-not $hmiAdd -or -not $hmiAdd.pass) {
        throw "HMI_UNIFIED_MISSING: Failed to add an HMI device. Ensure WinCC Unified option is installed (or use Classic HMI path), then retry."
    }

    # PLC: create a simple DB for symbolic binding
    $dbJson = @{
        dbName = 'DB_MotorData'
        dbNumber = 1
        staticMembers = @(
    ...
    # Unified HMI: connection + tags bound to PLC symbols
    ...
    if ($hinfo.pass) {
        try {
            ...
            if ($o.programType -and $o.programType -ne 'Unified') {
                throw "HMI_NOT_UNIFIED: HMI '$hmiSoftware' programType=$($o.programType). This test requires WinCC Unified HMI so PLC tags can be bound via EnsureUnifiedHmiTag/Connection."
            }
        } catch { throw }
    }
    ...
    # UI: screen + buttons; actions use HMI tags (which are PLC-bound)
    ...
    $design = @{
        ...
        items = @(
            @{ type='Button'; name='StartBtn'; left=56; top=120; width=220; height=96; text='启动'; properties=@{ BackColor='0xFF22C55E'; ForeColor='0xFFFFFFFF'; BorderColor='0xFF15803D'; BorderWidth=1 }; font=@{ Size=22 } }
            @{ type='Button'; name='StopBtn';  left=304; top=120; width=220; height=96; text='停止'; properties=@{ BackColor='0xFFDC2626'; ForeColor='0xFFFFFFFF'; BorderColor='0xFFB91C1C'; BorderWidth=1 }; font=@{ Size=22 } }
    ...
    $md = @()
    $md += "# Cold-start Unified PLC↔HMI Link Verification"
    $md += ""
    ...
    foreach ($r in $script:results) {
        # Markdown table needs '|' escaped. Avoid literal backslash in source to
        # prevent encoding/escape parsing issues on some Windows shells.
