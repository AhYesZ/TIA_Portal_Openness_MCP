...
# MCPVerify_FC_SCL_v3.scl — REPEAT..UNTIL + IF/ELSIF/ELSE on 安全PLC only.
...
$srcScl = Join-Path $tiaportalRoot 'skill\scl-cookbook\MCPVerify_FC_SCL_v3.scl'
...
$scl = Join-Path $staging 'MCPVerify_FC_SCL_v3.scl'
Copy-Item -LiteralPath $srcScl -Destination $scl -Force
$reportMd   = "$PSScriptRoot\verify_scl_v3.md"
$reportJson = "$PSScriptRoot\verify_scl_v3.json"
...
try {
    ...
    Send-Request 'initialize' @{ protocolVersion='2024-11-05'; capabilities=@{}; clientInfo=@{ name='verify-scl-v3'; version='1.0' } } 30000 | Out-Null
    ...
    Verify -category 'L2-PLC' -label "DeletePlcExternalSource(MCPVerify_FC_SCL_v3.scl)" -toolArgs @{
        softwarePath=$plc; externalSourceName='MCPVerify_FC_SCL_v3.scl'
    } -timeoutMs 60000 -toolName 'DeletePlcExternalSource' | Out-Null

    Verify -category 'L2-PLC' -label "ImportPlcExternalSource(MCPVerify_FC_SCL_v3.scl)" -toolArgs @{
    ...
    Verify -category 'L2-PLC' -label "GenerateBlocksFromExternalSource(MCPVerify_FC_SCL_v3.scl)" -toolArgs @{
        softwarePath=$plc; externalSourceName='MCPVerify_FC_SCL_v3.scl'
        ...
        try {
            ...
            if ($ec -gt 0) {
                [void]$script:results.Add([pscustomobject]@{
                    ...
                    detail = "errorCount=$ec (SCL v3 must compile clean)"
                })
            }
        } catch { }
    }
    ...
    $md = @()
    $md += "# SCL external source v3 (Delete + REPEAT..UNTIL + IF/ELSIF/ELSE)"
    $md += ""
