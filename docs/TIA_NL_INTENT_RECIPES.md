...
## 12. 从参考目录 Seed PLC 程序到新项目

**用户说**: "把参考项目里的标准 FB 库导入到新建的这个项目"

**工具序列**:
```
1. GetSoftwareTree(softwarePath="PLC_1")   ← 确认目标 PLC 结构
2. SeedProjectFromReference(
     softwarePath="PLC_1",
     referenceDirectory="<参考目录路径>",
     compile=true
   )
   ← 自动导入 UDT/TagTable/Block，按依赖顺序
3. CompileAndDiagnosePlc(softwarePath="PLC_1")
4. SaveProject
```
...
**备用（手动批量）**:
```
2a. ImportPlcProgramFromDirectory(
      softwarePath="PLC_1",
      importDirectory="<参考目录>",
      compile=true
    )
```

**成功判断**: 编译 errors=0，GetSoftwareTree 中出现参考目录中的块名
