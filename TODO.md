...
- [x] Implement `RunHttpHost` (MVP) using `HttpListener` on .NET Framework 4.8
  ...
  - Forward request body to MCP handler via `WithStreamServerTransport` + `McpBlockingStream`
  - Map validation errors to `400`, notifications to `202`, unexpected to `500`
