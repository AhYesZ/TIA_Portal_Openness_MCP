...
| `plcbuild-json/*.json` | `PlcBuildAndImport` 入参模板 |
| `lad-recipes/lad_call_recipes.json` | `BuildFlgNetCallXml` LAD 调用网络配方 |
| `scl-examples/FC_InstructionGallery.scl` | SCL 外部源示例 |
...
| `fb_basic_latch.json` | fb | 布尔保持和复位 |
| `fb_timer_counter_demo.json` | fb | TON 定时、上升沿计数、复位 |
| `fb_step_sequence_demo.json` | fb | CASE 步序状态机 |
| `fc_basic_scale_limit.json` | fc | 线性缩放和 LIMIT 限幅 |
| `fc_math_compare_demo.json` | fc | ABS、比较、误差、限幅输出 |
...
推荐导入顺序：TagTable -> UDT -> GlobalDB -> FC -> FB -> LAD/XML 样例。
