    ...
    public partial class Portal
    {
        ...
        public bool ImportPlcTagTable(string softwarePath, string folderPath, string importPath)
        {
            ...
            try
            {
                ...
                if (TryImportEngineeringObjectIntoCollection(tables, PrepareXmlForImport(importPath), out _, out var err)) return true;
                LastImportError = err;
                return false;
            }
            catch (Exception ex)
            ...
            return RunHmiStepTool("ApplyUnifiedHmiScreenDesignJson", meta =>
            {
                ...
                foreach (var itemNode in itemArray.OfType<JsonObject>())
                {
                    ...
                    try
                    {
                        ...
                        {
                            ApplyJsonProperties(item, props, failed, name!, typeHint);
                        }
        ...
        private static void ApplyJsonProperties(object target, JsonObject props, JsonArray failed, string path, string? itemTypeHint = null)
        {
            foreach (var kv in props)
            {
                if (!IsUnifiedHmiPropertyAllowed(target, itemTypeHint, kv.Key, path, failed))
                {
                    continue;
                }
        ...
        private static bool IsUnifiedHmiPropertyAllowed(object target, string? itemTypeHint, string propertyName, string path, JsonArray failed)
        {
            var prop = propertyName?.Trim() ?? string.Empty;
            if (string.IsNullOrWhiteSpace(prop)) return false;
            var normalizedType = NormalizeUnifiedHmiItemTypeHint(itemTypeHint, target);
            if (normalizedType == "rectangle")
            {
                var allowed = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
                {
                    "BackColor", "BackgroundColor", "FillColor", "BorderColor", "BorderWidth",
                    "BorderStyle", "Visible", "Opacity", "Left", "Top", "Width", "Height"
                };
                if (!allowed.Contains(prop))
                {
                    failed.Add($"{path}.{prop}: unsupported for Rectangle; put text/font/ForeColor on a Text item layered above the rectangle");
                    return false;
                }
            }
            else if (normalizedType == "iofield")
            {
                var allowed = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
                {
                    "BackColor", "ForeColor", "BorderColor", "BorderWidth", "Visible", "Opacity",
                    "Left", "Top", "Width", "Height", "ProcessValue", "Format", "DisplayFormat"
                };
                if (!allowed.Contains(prop))
                {
                    failed.Add($"{path}.{prop}: unsupported for IOField by the stable schema");
                    return false;
                }
            }
        ...
        private static string NormalizeUnifiedHmiItemTypeHint(string? itemTypeHint, object target)
        {
            var raw = (itemTypeHint ?? target.GetType().Name ?? string.Empty).Trim();
            if (raw.IndexOf("Rectangle", StringComparison.OrdinalIgnoreCase) >= 0) return "rectangle";
            if (raw.IndexOf("IOField", StringComparison.OrdinalIgnoreCase) >= 0 || raw.IndexOf("IoField", StringComparison.OrdinalIgnoreCase) >= 0) return "iofield";
            if (raw.IndexOf("Button", StringComparison.OrdinalIgnoreCase) >= 0) return "button";
            if (raw.IndexOf("Text", StringComparison.OrdinalIgnoreCase) >= 0) return "text";
        ...
        /// </summary>
        public bool DeletePlcExternalSource(string softwarePath, string externalSourceName)
        {
            LastPlcGenError = "";
            if (IsProjectNull())
            {
                LastPlcGenError = "DeletePlcExternalSource: project is null";
                return false;
            }

            if (string.IsNullOrWhiteSpace(externalSourceName))
            {
                LastPlcGenError = "DeletePlcExternalSource: externalSourceName is empty";
                return false;
            }
            ...
            {
                LastPlcGenError = $"DeletePlcExternalSource: PlcSoftware not found at '{softwarePath}'";
                return false;
            }
            ...
            if (sources == null)
            {
                LastPlcGenError = "DeletePlcExternalSource: ExternalSources collection not available";
                return false;
            }
            ...
            foreach (var item in sources)
            {
                ...
                try
                {
                    ...
                    LastPlcGenError = $"DeletePlcExternalSource: no parameterless Delete() on {item.GetType().Name}";
                    return false;
                }
                catch (Exception ex)
                {
                    LastPlcGenError = $"DeletePlcExternalSource: {ex.Message}";
                    return false;
                }
            }
        ...
        public bool ImportPlcExternalSource(string softwarePath, string groupPath, string filePath)
        {
            if (IsProjectNull())
            {
                LastPlcGenError = "ImportPlcExternalSource: project is null";
                return false;
            }
            var softwareContainer = GetSoftwareContainer(softwarePath);
            if (softwareContainer?.Software is not PlcSoftware plcSoftware)
            {
                LastPlcGenError = $"ImportPlcExternalSource: PlcSoftware not found at '{softwarePath}'";
                return false;
            }
            ...
            if (group == null)
            {
                LastPlcGenError = $"ImportPlcExternalSource: ExternalSourceGroup not found (groupPath='{groupPath}')";
                return false;
            }
            ...
            if (!fi.Exists)
            {
                LastPlcGenError = $"ImportPlcExternalSource: file not found '{filePath}'";
                return false;
            }
            ...
            LastPlcGenError = "ImportPlcExternalSource: all import-like methods failed: " + string.Join(" | ", failures.Take(12));
            return false;
        }
        ...
        public bool GenerateBlocksFromExternalSource(string softwarePath, string externalSourceName)
        {
            if (IsProjectNull()) return false;
            var softwareContainer = GetSoftwareContainer(softwarePath);
            if (softwareContainer?.Software is not PlcSoftware plcSoftware) return false;
            ...
            LastPlcGenError = "GenerateBlocksFromExternalSource: " + string.Join(" | ", failures.Take(10));
            return false;
        }
        ...
        public bool ImportFromDocuments(string softwarePath, string groupPath, string importPath, string fileNameWithoutExtension, ImportDocumentOptions option)
        {
            ...
            try
            {
                ...
                if (softwareContainer?.Software is PlcSoftware plcSoftware)
                {
                    ...
                    {
                        LastPlcGenError = $"ImportFromDocuments('{fileNameWithoutExtension}') threw: {ex.Message}";
                        throw;
                    }
                    ...
                    if (result != null)
                    {
                        LastPlcGenError = $"ImportFromDocuments('{fileNameWithoutExtension}') result state: {result.State}";
                    }
                }
            }
            catch (Exception ex)
