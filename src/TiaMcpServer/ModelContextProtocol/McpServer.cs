...
namespace TiaMcpServer.ModelContextProtocol
{
    ...
    public static partial class McpServer
    {
        ...
        public static ResponseMessage ApplyUnifiedHmiScreenDesignJson(
            ...
            [Description("strict: when true, unsupported item properties or failed writes make the tool fail instead of returning a false success.")] bool strict = true)
        {
            try
            {
        ...
        private static PlcBuildCapabilityDecision AnalyzePlcBuildCapability(string kind, string json)
        {
            ...
            for (var i = 0; i < operations.Count; i++)
            {
                ...
                var kindText = op["op"]?.ToString()?.Trim().ToLowerInvariant() ?? "";
                ...
                if (kindText == "line" && op["items"] is JsonArray items)
                {
                    foreach (var item in items.OfType<JsonObject>())
                    {
                        var token = item["token"]?.ToString() ?? item["raw"]?.ToString() ?? "";
                        if (IsComplexSclToken(token))
                            risky.Add($"$.structuredText.operations[{i}].items token '{token}'");
                    }
                }
            }
            ...
            if (risky.Count > 0)
            {
                ...
                result.Warnings.Add("Complex SCL constructs detected. The XML DSL is intentionally narrow and may generate XML that imports but fails TIA compile.");
                result.Warnings.AddRange(risky.Take(12));
                result.NextActions.Add("Generate native SCL/SIMATIC document source and import it through ImportBlockFromScl/ImportBlocksFromScl or a verified external-source workflow.");
                result.NextActions.Add("If you keep XML DSL, import only into a disposable project and require CompileAndDiagnosePlc errors=0 before saving.");
            }
            else
            {
                result.NextActions.Add("XML DSL appears within the verified narrow subset. Dry-run first, then import and require CompileAndDiagnosePlc errors=0.");
            }

            return result;
        }

        private static bool LooksLikeSclExpression(string text)
        {
            if (string.IsNullOrWhiteSpace(text)) return false;
            var t = text.Trim();
            if (t.StartsWith("\"", StringComparison.Ordinal) && t.EndsWith("\"", StringComparison.Ordinal)) return false;
            if (t.IndexOf(' ') >= 0) return true;
            return t.IndexOfAny(new[] { '+', '-', '*', '/', '<', '>', '=', '(', ')', ',' }) >= 0
                || t.IndexOf("AND", StringComparison.OrdinalIgnoreCase) >= 0
                || t.IndexOf("OR", StringComparison.OrdinalIgnoreCase) >= 0
                || t.IndexOf("NOT", StringComparison.OrdinalIgnoreCase) >= 0
                || t.IndexOf("MOD", StringComparison.OrdinalIgnoreCase) >= 0;
        }

        private static bool IsComplexSclToken(string token)
        {
            if (string.IsNullOrWhiteSpace(token)) return false;
            var t = token.Trim();
            return string.Equals(t, "CASE", StringComparison.OrdinalIgnoreCase)
                || string.Equals(t, "FOR", StringComparison.OrdinalIgnoreCase)
                || string.Equals(t, "WHILE", StringComparison.OrdinalIgnoreCase)
                || string.Equals(t, "REPEAT", StringComparison.OrdinalIgnoreCase)
                || string.Equals(t, "MOD", StringComparison.OrdinalIgnoreCase);
        }
